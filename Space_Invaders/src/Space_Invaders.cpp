...
void myIlumination()
{
    ...
    case 1:
        ...
        break;
    default:
        glLightModelfv(GL_LIGHT_MODEL_AMBIENT, amb_temp);
        glEnable(GL_LIGHT0);
        glDisable(GL_LIGHT1);
        ilumination = 1;
    }
...
{
    if (key - 'a' == 0) {
        game->canonLeft(glutGet(GLUT_ELAPSED_TIME));
    } else if (key - 'd' == 0) {
        game->canonRight(glutGet(GLUT_ELAPSED_TIME));
    } else if (key - ' ' == 0) {
        game->fireWeapon();
    } else if (key - '0' < 10 && key - '0' >= 0) {
        game->cameraChange(key - '0');
    } else if (key - 'l' == 0) {
        myIlumination();
    } else if (key - 27 == 0) {
        exit(0);
