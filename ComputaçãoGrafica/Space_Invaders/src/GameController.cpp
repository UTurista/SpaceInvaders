...
GameController::GameController(float left, float right):left(true),leftLimit(left),rightLimit(right), _activeCamera(0){}
...
void GameController::canonRight(float timeCalled){
	if((canon->getOffset()+canon->getLenght())<rightLimit){canon->moveRight(timeCalled);}	//Alem da posição do Canhão é necessario somar a distancia
...
void GameController::canonLeft(float timeCalled){
	if(canon->getOffset()-canon->getLenght()>leftLimit){canon->moveLeft(timeCalled);}		//Alem da posição do Canhão é necessario subtrair a distancia
...
void GameController::tick(){
	if(left){
		...
		if((ovni->getPosX()-1)<=leftLimit){checkDownMov();}
	}else{
		Alien *ovni = invaders[59];
		if((ovni->getPosX()+12)>=rightLimit){checkDownMov();}
	}
...
int  GameController::aliensFireWeapon(){
	...
	if(!invaders[counter]->getLife()){return 2;}	//Se o Alien escolhido está 'morto' algoritmo termina

	
	if(invaders[counter]->getPosY()==invaders[59]->getPosY()){	//Se o Alien escolhido está na ultima linha
				if((rand()%50)<PROBABILIDADE){invaders[counter]->fireWeapon();}
				return 0;
	}
	...
	while(x+counter+1<60 &&	(invaders[counter+x]->getPosY()>invaders[counter+x+1]->getPosY())){
		if(invaders[counter+x+1]->getLife()){colunaLivre=false;break;}//Verifica-se se o proximo está morto
		x++;
	}
	/***** *************************************************** ******/
...
void GameController::colliderManager(){
	...
	for (int i=0; i<allBullets[0].size(); i++){
		...
		if(!allBullets[0][i]->isDrawing()){continue;}//Se a bala não está a ser desenhada nem vale apena continuar, Next!
	...
	for(int i=1; i<=60; ++i){
		for(int j=0; j<allBullets[i].size(); j++){
			if(!allBullets[i][j]->isDrawing()){continue;}//Se a bala não está a ser desenhada nem vale apena continuar, Next!
